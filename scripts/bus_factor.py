...
def load_contrib():
    """repo -> {login: contributions}, and curated is-bot flag per login.

    GitHub's contributor endpoint sometimes reports automation accounts as
    ``User`` (notably ``actions-user``).  The maintainer-reviewed identity map
    is therefore authoritative when it marks an account as a bot.
    """
    ...
    with open(CONTRIB, encoding='utf-8') as f:
        for row in csv.DictReader(f):
            ...
            is_bot[login] = (
                row['type'].strip().lower() == 'bot'
                or bool(cmap.get(login, {}).get('is_bot'))
            )
...
def main():
    ...
    if os.path.exists(PEOPLE):
        with open(PEOPLE, encoding='utf-8') as f:
            for row in csv.DictReader(f):
                if not row['login'] or is_bot.get(row['login'], False):
                    continue
                n_people += 1
    ...
    A('_Generated by `scripts/bus_factor.py` from `observatory/site/src/data/'
      'contributors.csv` plus the maintainer-reviewed bot overrides in '
      '`scripts/contributors_map.json` (offline, reproducible). Bots excluded '
      'from continuity risk; contributions = GitHub commit-contribution counts '
      'per repo._')
    A('')
    A('## Headline')
    A('')
