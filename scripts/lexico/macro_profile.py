...
# Indigenous citation style (SKD, VCP, AMAR, …): quotations in “…” attributed to
# abbreviated authorities ending in '0' (jE0=Jaimini, BA0=Bhāṣya, amara0=Amara …)
# and closed with 'iti'. These dicts cite heavily but use NO <ls> tag.
RX_QUOTE = re.compile(r'“')
RX_ABBR0 = re.compile(r'[A-Za-z]0')


def sample_entries(path, n):
    """Yield ~n entries spread evenly across the WHOLE file (stratified), not the
    first n — first-N skews to short early-alphabet entries (e.g. it misses the
    big mid-alphabet entries like dharma)."""
    total = 0
    ...
    if total == 0:
        return
    step = max(1, total // n)
    idx, collecting, cur, taken = -1, False, [], 0
    with open(path, 'r', encoding='utf-8-sig', errors='replace') as f:
        for line in f:
            line = line.rstrip('\n')
            if line.startswith('<L>'):
                if collecting and taken < n:
                    yield '\n'.join(cur); taken += 1
                idx += 1
                collecting = (idx % step == 0)
                cur = [line] if collecting else []
            elif collecting:
                cur.append(line)
    if collecting and taken < n:
        yield '\n'.join(cur)
...
def profile(path, n):
    ...
    for entry in sample_entries(path, n):
        e += 1
        chars += len(entry)
        c = len(RX_LS.findall(entry)) + len(RX_QUOTE.findall(entry))   # Western <ls> + indigenous “…”
        cites += c
        if c or RX_ABBR0.search(entry): withcite += 1
...
<h1>Structural profile of {len(rows)} CDSL dictionaries ({N} entries sampled across each, stratified)</h1>
...
Citation = Western <code>&lt;ls&gt;</code> tag OR indigenous “…” quotation (SKD/VCP/AMAR cite via quotations +
abbreviated authorities, with no <code>&lt;ls&gt;</code>). Heuristic detectors; cross-dict numbers indicative, not exact. See
...
{''.join(svg)}
    ...
    print(f'profiled {len(rows)} dicts ({N} entries sampled stratified across each)')
