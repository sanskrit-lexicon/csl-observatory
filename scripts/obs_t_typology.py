...
def write_report(s, dict_rows, corr_rows, form_cons):
    ...
    A('_Generated by `scripts/obs_t_typology.py` from `correction_events_final.csv` '
      '(the unified, component-attributed correction-event table). Sanskrit spans '
      'are normalized to IAST where available; raw source/markup edits keep their '
      '`edit_space` audit marker. '
      'Built offline from the correction-form export, the `../csl-orig` git history, '
      'and sibling `../CORRECTIONS` files. Every event carries an evidence label '
    ...
    A('> **Claim (two axes).** Across twelve years and 43 dictionaries, each '
      ...
      'spacing, source-raw, …). The largest derived location is **sense**; markup, '
      'headword, and citation form the next tier. Minor-edit rates vary sharply by '
      'location, so surface-edit claims are reported by location rather than '
      'asserted globally. The location profile differs by dictionary and shifts over time.')
    A('')
    ...
    A('')
    A('The largest derived location is **sense (definition)**. Markup, headword, '
      'and citation form the next tier, so location claims should be read from the '
      'derived table rather than collapsed into a meaning-bearing vs markup contrast.')
    A('')
    A('## 2. Axis B — edit type')
    A('')
    A('What kind of change each correction is (from the edit-op trace, all events). '
      '`edit_space` separates IAST Sanskrit spans from raw SLP1/source and markup '
      'edits; raw source edits are not counted as Sanskrit case/spelling changes. '
      'There is no automatic "content rewrite" category.')
    A('')
    et = s.get('editType', [])
    ...
    A('## Draft abstract')
    A('')
    A(f'> We present a twelve-year, {s["events"]:,}-event corpus of corrections to the Cologne '
      f'Digital Sanskrit Lexicon ({s["dictionaries"]} dictionaries) and a two-axis typology of the '
      'errors they repair. Unifying a 2014–2019 correction-form archive with the '
      '2019–2026 source git history, we normalize Sanskrit spans to IAST (handling '
      'the form\'s mixed Devanagari/Harvard-Kyoto encoding) while retaining raw '
      'source strings for audit, and describe each event by its '
      '**location** in the dictionary microstructure and its **edit type**. '
      'The largest derived location is sense, followed by markup, headword, and '
      'citation; minor-edit rates vary sharply by location; the location profile '
      'differs markedly by dictionary; and a stable '
      'character-confusion signal (led by b/v in the form layer) emerges. We release the corpus with '
