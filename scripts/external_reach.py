...
def main() -> None:
    ...
    add("zenodo", f"record {ZENODO_RECORD_ID}", "status", zstatus,
        ...
        "measured" if zstatus == "ok" else "blocked", zdetail)
    ...
    L: list[str] = []
    A = L.append
    A(f"_Created: 03-07-2026 · Last updated: {datetime.now(timezone.utc).strftime('%d-%m-%Y')}_")
    A("")
    ...
    A(f"| Zenodo OBS-T download stats | **{zstatus.upper()}** | "
      + ("record verified — downloads counted below" if zstatus == "ok"
         else "see blocker below") + " |")
    A("")
    ...
    A("## Zenodo OBS-T download stats — "
      + {"ok": "OK (record verified)",
         "mismatch": "BLOCKED (DOI mismatch)",
         "unavailable": "UNAVAILABLE"}.get(zstatus, zstatus.upper()))
    A("")
    if zstatus == "mismatch":
    ...
    if zstatus == "ok":
        A("- **The OBS-T Zenodo DOI is verified real** (concept "
          f"`10.5281/zenodo.21346705`) — the paper may cite it, and this tier "
          "counts the dataset's own downloads.")
    else:
        A("- **Fix the OBS-T DOI before the paper ships** — the current DOI "
          "points at someone else's work.")
    A("")
