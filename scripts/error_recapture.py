...
def load_calibrated():
    """Below-floor dictionaries with a corrected band, split by whether it capped.

    Written by `scripts/lowm_estimators.py` (H3986). Read rather than restated so
    the two reports cannot drift apart; absent file degrades to a pointer.
    """
    path = os.path.join(DATA, 'error_recapture_calibrated.csv')
    if not os.path.exists(path):
        return [], []
    free, capped = [], []
    with open(path, encoding='utf-8') as f:
        for r in csv.DictReader(f):
            (capped if r.get('capped') == '1' else free).append(r['dict'])
    return free, capped
...
def main():
    ...
    L = []; A = L.append
    # Dated header, emitted rather than hand-added: a docs pass added one to the
    # committed report in Aug 2026 and every regeneration since would have
    # silently stripped it again.
    from datetime import date
    A('_Created: 03-07-2026 · Last updated: %s_' % date.today().strftime('%d-%m-%Y'))
    A('')
    ...
    A('An **independent second design** — correctors within one era as the capture '
      ...
      '[`corrector_recapture.md`](https://github.com/sanskrit-lexicon/csl-observatory/blob/main/reports/corrector_recapture.md). It is the only handle on '
    ...
    A('**How much of each figure is the assumption?** Violations (1) and (2) both '
      ...
      'figures here with that envelope, not with the CI alone.')
    A('')
    ...
    A('')
    A(f'### What the m >= {MIN_M} floor is, and what it is not')
    A('')
    A('That threshold has been tested against this corpus rather than assumed, by '
      'thinning the three dictionaries with an uncapped estimate until their '
      'recaptures fall below it and asking what the estimator then does: '
      '[`error_recapture_lowm.md`](https://github.com/sanskrit-lexicon/csl-observatory/blob/main/reports/error_recapture_lowm.md) '
      '(`scripts/lowm_estimators.py`). Three results bear on how the table above '
      'should be read.')
    A('')
    A(f'1. **No estimator that borrows strength across dictionaries reproduces the '
      f'known answers.** Pooled-prevalence and correction-density extrapolation — '
      f'the two candidates that are defined where m = 0 — land between x0.35 and '
      f'x3.4 of Chapman on pw, mw and bur, because error-site prevalence across '
      f'those three spans a factor of 4.1 and exchangeability across dictionaries '
      f'is therefore false. Chao2 over the two eras and Chao2 over correctors fail '
      f'their own scorings (x1.02-x2.33 and x0.96-x2.17). **For the dictionaries '
      f'with m = 0 the observed count in the table above is not a placeholder '
      f'awaiting a better formula — it is the answer, as a lower bound.**')
    A(f'2. **The floor is a precision convention, not an identifiability '
      f'boundary.** Below it Chapman is median-shifted by a factor that depends on '
      f'the sample size and barely on the dictionary (x0.54 at m = 1, x0.69 at '
      f'm = 2, x0.88 at m = 5; the three agree to within 9% at every rung). '
      f'Dividing that measured shift out yields corrected estimates with 5-95% '
      f'bands for the dictionaries that have at least one recapture.')
    free, capped_low = load_calibrated()
    if free or capped_low:
        A('3. **That correction reaches %d of them** — %s — in '
          ...
          '%d more (%s) have their corrected estimate capped at their own record '
          'count, which carries the same reading as cae above: treat the dictionary '
          'as unproofread, not as carrying a number.'
          % (len(free), ', '.join('`%s`' % d for d in free), len(capped_low),
             ', '.join('`%s`' % d for d in capped_low)))
    else:
        A('3. The per-dictionary corrected bands live in '
          '`observatory/site/src/data/error_recapture_calibrated.csv`; run '
          '`python scripts/lowm_estimators.py` to regenerate them.')
    A('')
    A('## Sensitivity')
    A('')
    ...
    if cross:
        ...
        A('**Cross-check against the within-era design.** '
          '[`corrector_recapture.md`](https://github.com/sanskrit-lexicon/csl-observatory/blob/main/reports/corrector_recapture.md) estimates the same '
    ...
    A('')
    # Emitted, not hand-added, for the same reason as the dated header above.
