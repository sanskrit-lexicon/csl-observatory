...
const rows = all.filter(d => d.layer === "L2");
```
...
rows where `layer = L2`, aggregated from kosha, SanskritRussian, SanskritLexicography,
and VisualDCS. **n = ${n}**. As of 06–13-07-2026.
:::
...
> **How to read:** log-scale bar of the kosha.db and DCS/Heritage/vidyut form
> counts. **Example:** kosha.db's 6.9M inflections is the largest single number
> in the whole census register — one entry (444,773) fans out into ~15 forms
> each on average once every paradigm cell is generated.
...
Paradigm-cell coverage moved from ◐ partial to quantified org-wide under H817
WS1.2 (VisualDCS browser data, now census-registered). Form→lemma ambiguity
rate is genuinely ○ not started — Q2 analytical-layer work.
