...
`refresh-observatory.yml` pipeline — last successful run 2026-08-31; that
pipeline has failed every scheduled run since 2026-09-07 (`data_index.py
--check`: "missing catalog entries: error_recapture_calibrated.csv,
error_recapture_lowm.csv"), so these five rows still reflect the 76-repo
snapshot until that separate failure is fixed._
