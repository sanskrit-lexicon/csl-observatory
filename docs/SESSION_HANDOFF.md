...
- Remaining (human/scope): **Q7 taxonomy FULLY DONE 2026-05-31** — 24 dictionary repos (786 issues) + 4 tooling repos csl-apidev/cologne-stardict/GreekInSanskrit/ArabicInSanskrit (153 issues) all verified all-clean (type+severity+milestone+project). A2 closed. **M3 full-runbook README/CLAUDE DONE 2026-05-31** for 13 stub repos (WIL + ACC, BEN, BHS, CAE, CCS, LRV, MCI, SCH, SKD, STC, VCP, VEI), matching the BOR-group gold standard (live counts, validated Mermaid, source bibliography, data-format example); pushed to each repo's default branch. **2 deferred:** ApteES (reverse-direction markup → B5), SHS (authorship conflict → B4). NB: the org-wide `docs-pass` branches are a *separate* microanalysis effort (DICT_PROFILE.md etc.), still awaiting maintainer review — not M3. Other remaining: M1 refresh-script modernization, M4 refresh automation (needs token), M7 KRM license decision, #51 XML-parse tail.
