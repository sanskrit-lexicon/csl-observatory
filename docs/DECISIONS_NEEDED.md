...
| D4 | **Taxonomy drift watch** ([DRIFT_WATCH.md](https://github.com/sanskrit-lexicon/csl-observatory/blob/main/docs/DRIFT_WATCH.md)): OK to (a) enable the weekly `dict-audit.yml` backstop now (reuses `TOOLING_AUDIT_TOKEN`), and (b) pilot the event-driven `needs-triage` guard on `GRA` before fanning out to all repos? | Awaiting M.G. decision |
...
| RH4-pilot | `csl-observatory` line-ending policy tightened to explicit LF for source, docs, and generated data files. | `.gitattributes` |
| RH4 | Org-wide rollout **complete 2026-07-27**: all 34 repos in `.github/workflows/tooling-audit.yml` REPOS now carry `eol=lf` `.gitattributes` — 12 already had it (`csl-observatory`, `hwnorm1`, `csl-pywork`, `hwnorm2`, `csl-app`, `csl-lslink`, `csl-atlas`, `csl-orig`, `csl-apidev`, `csl-websanlexicon`, `csl-corrections`, `cologne-stardict`); 22 rolled out and merged via `cologne_batch_deploy.py --deploy gitattributes` (`MWinflect`, `alternateheadwords`, `csl-devanagari`, `mw-dev`, `csl-inflect`, `csl-ldev`, `csl-homepage`, `literarysource`, `csl-doc`, `csl-newsletter`, `csl-westergaard`, `csl-kale`, `csl-lnum`, `csl-sqlite`, `avlinks`, `rvlinks`, `csl-whitroot`, `csl-json`, `csl-santam`, `sanskrit-fonts`, `cologne-hugo`, `sanskrit-lexicon.github.io`). | [H1542](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H1542-Sonnet_csl-observatory_rh4-gitattributes-lf-rollout_23.07.26.md); [`cologne_batch_deploy.py`](https://github.com/gasyoun/Uprava/blob/main/tools/cologne_batch_deploy.py) |
