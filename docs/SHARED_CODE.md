...
|---|---|---|---|---|
| 1 | IAST ⇄ SLP1 ⇄ Devanāgarī transcode | `sanskrit-util/` (**new**) | extracted, tested | `import`/require it; don't re-type the SLP1 table |
| 2 | Sanskrit normalization keys (`norm`/`nfold`/`form_key`) | `sanskrit-util/` (**new**) | extracted, tested | same package; pick the documented key |
| 3 | Dict correction pipeline (`updateByLine`, `diff_to_changes*`, `make_xml`, `parseheadline`, `digentry`) | `csl-pywork/v02/makotemplates/pywork/` | already shared (vendored) | copy from csl-pywork; never fork-and-edit |
| 4 | `<ls>` citation linking + verb/preverb morphology | MWS `link_candidates.py` + `verbs01/` | **15 forks, no shared module** | see §4 before writing a new link-splitter |
| 5 | PHP dict-web endpoints (`getword`/`servepdf`/`serveimg`) | `csl-websanlexicon/v02/makotemplates/web/webtc/` | shared template; ~37 copies drift | fix the **template**, then regenerate |
| 6 | Repo hygiene / CI (CodeQL, dependabot, pre-commit, CoC, CONTRIBUTING) | `ci_templates/` + `cologne-*` skills | automated | run the skill; don't hand-write per repo |
| 7 | RU translation kits (`mw_ru` / `pwg_ru`) | `SanskritLexicography/RussianTranslation/` | shared scaffold | parameterize the one kit |
| 8 | DCS corpus / morphology ingest | VisualDCS `import_dcs_conllu.py` | 5 forks | see §8 |
...
`sanskrit-util/README.md`.

| Was re-implemented in | As | Now |
|---|---|---|
| 20+ dict repos + ~50 CORRECTIONS/dictionaries/*/transcoder.py | `import transcoder` (Dhaval's FSM engine, 62 copies) | keep for **dict-build** work; for app code use `sanskrit-util` |
| WhitneyRoots/scripts/sanskrit_util.py | `to_slp1`/`from_slp1`/`form_key`/`norm`/`nfold` | **donor → now a shim re-exporting the package** ✅ |
| WhitneyRoots/reader/reader.js | inline `deva2iast`/`norm`/`nfold` | folded into the package (JS port) |
| WhitneyRoots/src/utils/linguistics.js | `normalizeSanskrit` (lossy) / `iastToDevanagari` | folded in as `normalize_sanskrit` / `iast_to_devanagari` |
| **BookIndex**/src/utils/linguistics.js | **a direct copy of WhitneyRoots' linguistics.js** | next consumer — highest-value dedup (verbatim fork) |
| csl-atlas/src/lib/lookup-normalize.js + scripts/lib/dict-normalize.mjs | own JS normalizers | **transcoders migrated** (csl-atlas PR #119); `dict-normalize` stays local (CDSL SLP1-headword-specific). (No `csl-atlas-gpu` repo exists as of 2026-06-15 — earlier mention was speculative.) |
| GRA/vn/gra-dev/web/webtc1/transcoderjs/transcoder3.js | a JS FSM transcoder | next consumer (JS build) |
| csl-app/lib/core/transliteration_service.dart | **Dart** `indic_transliteration_dart` wrapper | leave as-is (Dart; no shared port) |
| SanskritSpellCheck/detectors/slp1util.py | SLP1 alphabet + confusion model | next consumer (share alphabet, keep model local) |
| SamudraManthanam/.../morph_service.py, slug.py, corpus_builder/html_to_canonical.py | own translit + normalize | next consumers |
| RussianRamayana/web/transliterate_filenames.py, VCP/meld_regex/convert_transliteration.py, csl-observatory/scripts/obs_t_translit_check.py, RWS-plugin/.../translit_lint.py | mixed (own / `indic_transliteration`) | next consumers |
| ApteES/.../hwnorm1.py, CORRECTIONS/.../hwchk_iast.py, csl-apidev/.../word_frequency_norm.py, IndologyScholars/keyword_filtering.py | headword/key normalizers | next consumers |
...
makotemplates originals and follow the [csl-orig correction workflow](../../Uprava-h4060-drain/CLAUDE.md)
(copy → edit → copy back → XML-validate → audit change file).

**Rule:** pull the script from csl-pywork — `v00/makotemplates/`
for `updateByLine.py` / `parseheadline.py`, and `v02/` for `generate_dict.sh` /
...
lift from — are MWS `link_candidates.py`
and MWS `link_expand.py` for linking, and
PWG `verbs01/mwverb.py` / `preverb1.py` for verb
...
**once per dictionary** under csl-websanlexicon/webbackup/ (40+
dicts) plus csl-apidev and GRA/vn.
The JSONP-XSS / SQLi hardening done across these belongs in the **template**
csl-websanlexicon/v02/makotemplates/web/webtc/
...
protection are deployed org-wide by the `cologne-*` skills from `ci_templates/`
(`generic-ci.yml`, `dictionary-ci.yml`, `python-tool-ci.yml`, `laravel-ci.yml`) and
`coc_templates/`, via the root `.deploy_*.py` scripts. The census still found
...
`SanskritLexicography/RussianTranslation/` holds the
...
adding `4_korpus_proverka`) over one `src/build_src.py`.
**Also note** a stray top-level `pwg_ru_prompts/` at the GitHub root that
...
VisualDCS `import_dcs_conllu.py`, with
`gen_dcs_lemma_summary.py` producing the lemma-frequency
table that other repos vendor. Re-ingesters to consolidate onto it:
WhitneyRoots/scripts/dcs/extract_dcs.py,
MWS `ls_L_dcs2026.py` + `register_b_dcs.py`,
csl-atlas `import-dharmamitra-morphology.py`,
csl-apidev `build_wf_from_dcs.py` + `dcs_xref/build_xref.py`.
