...
Of 76 repositories, **67 (88%) have a bus factor of 1** — a single person accounts
for the majority of contributions — and no repository in the organisation reaches a
bus factor of 3. One maintainer (funderburkjim) alone carries 52.1% of all
contributions across 58 repositories; the core trio (funderburkjim, drdhaval2785,
gasyoun) carries **98.0%**. The Gini coefficient of the contribution distribution
is 0.856. One person covers 50% of all recorded work; two cover 80%; three cover
...
was executed by the same handful of people who carry 98.0% of all contributions
(and, per Section 3.7, by at most five content correctors in any year), plus
...
| 3 | Bus factor 1 in 67/76 repos; top contributor 52.1%; trio 98.0%; Gini 0.856; zero ORCIDs | [`reports/bus_factor.md`](https://github.com/sanskrit-lexicon/csl-observatory/blob/main/reports/bus_factor.md) | ✅ |
...
([`data/manifest.json`](https://github.com/sanskrit-lexicon/csl-observatory/blob/main/data/manifest.json)).
The committed analysis inputs backing this article are additionally archived on
Zenodo: concept DOI `10.5281/zenodo.22853817`, version DOI
`10.5281/zenodo.22853818` (v1.0.0, 2026-09-20) — the 67-file catalog indexed by
`data_index.csv` (76-repository snapshot, issue/commit ledgers, and the
52,498-event `correction_events_release.csv` correction ledger).
